        ...
        findings.append(
            {
                ...
                "status": "open",
                "category": "coverage",
                "evidence": "Durable Scanner outcome scoring filters to regular-session scans.",
                "risk": (
                    "Premarket/after-hours discoveries can be visible to the Scanner "
                    "but absent from the main learning feedback loop."
                ),
                "recommended_action": (
...
def audit_observations(observations: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ret60 = _num(row.get("return_60m_pct"))
        mfe60 = _num(row.get("mfe_60m_pct"))
        ...
        if ret60 is not None and mfe60 is not None and mfe60 >= 5.0 and ret60 < 3.0:
            item = {
                ...
                "mfe_60m_pct": mfe60,
                "mae_60m_pct": _num(row.get("mae_60m_pct")),
            }
    ...
    denominator = len(
        [
            row
            for row in rows
            if _num(row.get("return_60m_pct")) is not None
            and _num(row.get("mfe_60m_pct")) is not None
        ]
    )
...
def run_audit(root: Path = ROOT, report_dir: Path = DEFAULT_REPORT_DIR) -> dict[str, Any]:
    source_findings = audit_source_contracts(root)
    rows, report_count = load_outcome_observations(report_dir)
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            f"- Outcome reports loaded: **{payload.get('outcome_reports_loaded', 0)}**",
...
def main() -> int:
    ...
    payload = run_audit(ROOT, Path(args.report_dir))
    ...
    print("Open source findings: " + str(len(payload.get("source_findings") or [])))
    empirical = payload.get("empirical_opportunity_audit") or {}
    print(
