...
def card(c):
    ...
    news_html = (
        ...
        f'<div class="nv">{html.escape(news_text[:220])}</div></div>'
        if news_text
        else ""
    )
...
<div class="legend-box">
  ...
  <div class="legend-grid">
    <div class="legend-item">
      <div class="legend-term">VWAP</div>
      <div class="legend-def">Volume-Weighted Average Price for today's session. It is the average traded price weighted by share volume. ABOVE VWAP means the current price is trading above that average.</div>
    </div>
    <div class="legend-item">
      <div class="legend-term">Volume Pace</div>
      <div class="legend-def">Current volume compared with the stock's normal volume expected by this time of day. 2.00x means roughly twice the usual pace.</div>
    </div>
    <div class="legend-item">
      <div class="legend-term">IEX Spread</div>
      <div class="legend-def">The percentage gap between the current IEX bid and ask. Smaller is generally easier to trade; this reflects IEX quotes, not the full consolidated SIP market.</div>
    </div>
  </div>
</div>
""",
