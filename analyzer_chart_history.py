...
def load_chart_history(symbol, now, session_date, *, tradier_token="", alpaca_fetch=None, feed="sip"):
    ...
    Request 30 days; the separately validated active/reference session wins.
    Provider failure leaves that session and daily history usable.
    """
    source = "Tradier consolidated · 5-minute · regular sessions" if tradier_token else f"Alpaca {feed} · 5-minute · regular sessions"
    try:
        end = datetime.fromisoformat(str(session_date)[:10]).replace(tzinfo=ET)
        start = end - timedelta(days=30)
        identity = sha256(tradier_token.encode()).hexdigest()[:16] if tradier_token else feed
        key = f"chart-history-v1:{symbol}:{end.date()}:{identity}"
        cached = get_cached_context(key, 6 * 3600)
        if cached and cached.get("symbol") == symbol and cached.get("status") == "ok":
            return cached
        if tradier_token:
            rows = get_timesales_bars(symbol, tradier_token, start, end, interval="5min",
                                     session_filter="open", strict=True, timeout=12)
        elif alpaca_fetch:
            rows = alpaca_fetch(symbol, "5Min", start, end, limit=10000, feed=feed, complete=True)
        else:
        ...
        for row in rows:
            ...
            normalized[stamp] = dict(t=stamp, **values)
